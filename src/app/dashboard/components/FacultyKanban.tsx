  ...
  return (
    <div className="faculty-kanban-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 300px), 1fr))", gap: "20px" }}>
      {faculties.length === 0 ? (
        ...
        faculties.map((f) => (
          <div key={f._id} className="glass faculty-card" style={{ padding: "24px", borderRadius: "var(--radius-lg)", border: "1px solid hsl(var(--border-color))", position: "relative" }}>
            <div className="faculty-card-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: "16px" }}>
            ...
            <div className="faculty-card-footer" style={{ display: "flex", alignItems: "center", gap: "6px" }}>
